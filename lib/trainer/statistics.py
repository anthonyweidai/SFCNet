...
class Statistics(object):
    ...
    def batchUpdate(self, PredLabel: Tensor, TargetLabel: Tensor, Loss: Tensor, Epoch: int):
        ...
        AreaPred, AreaInter, AreaUnion, UnionCout = computeMaskRegion(PredLabel.detach().clone(), TargetLabel.detach().clone())
        ...
        if 'val' in self.State:
            self.Em += measureEAM(PredLabel.detach().clone(), TargetLabel.detach().clone())
            self.Ae += computeAE(PredLabel.detach().clone(), TargetLabel.detach().clone())
    ...
    def update(self):
        ...
        if 'smooth' in self.opt.sup_method:
            return
