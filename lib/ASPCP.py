...
@registerSegmentationHead("aspcp")
class DeeplabV3(BaseSegHead):
    """A sementic segmentation system
    Inspired by `DeepLabv3 architecture <https://arxiv.org/abs/1706.05587>`
    """
    def __init__(self, opt, **kwargs: Any) -> None:
        ...
        self.Aspcp = nn.Sequential()
        self.Aspcp.add_module(
            name="aspp_layer",
            module=ASPCP(
    ...
    def profileModule(self, FeaturesTuple: list) -> Tuple[Tensor, float, float]:
        ...
        x, p, m = moduleProfile(module=self.Aspcp, x=FeaturesTuple[-1])
        params += p
        macs += m
