...
class MainTrain(object):
    ...
    def filePathInit(self, opt):
        ...
        # log file
        self.ExpLogPath = "./%s/%s/%s/log_%s%s.csv" % (opt.exp_base, TaskType, opt.exp_level, opt.sup_method, "")
    ...
    def logFieldInit(self, opt):
        ...
        FGListName = [
            ...
            "FGForHead", "FGBottle", "FGBottleSE", 
            "FGNoStage5", "FGLink", 
        ]
        FGListValues = [
            opt.seg_feature_guide, opt.fg_use_guide, opt.fg_start_stage, opt.fg_resize_stage, 
            opt.fg_for_head, opt.fg_bottle, opt.fg_bottle_se, 
            opt.fg_nostage5, opt.fg_link, 
        ]
